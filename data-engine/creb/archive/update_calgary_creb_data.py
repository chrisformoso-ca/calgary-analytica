...
class CalgaryDataUpdater:
    ...
    def _parse_district_line(self, line: str, property_type: str, month: int, year: int, date_str: str) -> Optional[Dict]:
        """Parse a single district data line."""
        
        # Common Calgary districts to look for
        districts = ['City Centre', 'North', 'North East', 'North West', 'South', 'South East', 'West', 'East']
        
        # Find district in line
        district = None
        for d in districts:
        ...
        # Look for patterns like: district_name, number, number, percentage, number, number, price, percentage, percentage
        ...
        if len(numbers) >= 8:  # Need at least 8 data points for complete record
            try:
                ...
                months_supply = float(numbers[3].replace(',', ''))
                benchmark_price = int(numbers[4].replace(',', ''))
                yoy_change = numbers[5] if '%' in numbers[5] else f"{numbers[5]}%"
                mom_change = numbers[6] if '%' in numbers[6] else f"{numbers[6]}%"
                ...
                logger.debug(f"Could not parse district line: {line} - {e}")
...
def main():
    ...
    parser.add_argument('--csv-path', 
                       default='../../../data/extracted/Calgary_CREB_Data.csv',
                       help='Path to the Calgary CREB city-wide data CSV file')
    parser.add_argument('--district-csv-path',
                       default='../../../data/extracted/calgary_housing_master_dataset.csv',
